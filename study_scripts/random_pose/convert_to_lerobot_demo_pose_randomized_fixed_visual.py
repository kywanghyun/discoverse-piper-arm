...
def maybe_copy_training_metadata(source_data_dir: str, dataset_root: str):
    ...
    # sample-level metadata examples
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Pose-randomized fixed-visual Discoverse 데이터를 LeRobot 포맷으로 변환')
    parser.add_argument(
        '--data-dir',
        type=str,
        default=os.path.expanduser('~/lerobot/DISCOVERSE/data/piper_place_block_pose_randomized_fixed_visual_data'),
        help='원본 Discoverse 데이터 루트 경로',
    )
    parser.add_argument(
        '--dataset-root',
        type=str,
        default=os.path.expanduser('~/lerobot/DISCOVERSE/lerobot_dataset_pose_randomized_fixed_visual'),
        help='출력 LeRobot 데이터셋 경로',
    )
    parser.add_argument(
        '--repo-id',
        type=str,
        default='apple/piper_place_block_pose_randomized_fixed_visual',
