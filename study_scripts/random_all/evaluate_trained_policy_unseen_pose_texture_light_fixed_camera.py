...
class UnseenPoseTextureLightPolicyEvaluator:
    ...
    def _capture_camera_poses(self) -> Dict[str, Dict[str, Any]]:
        ...
        for cam_name in self.camera_cfgs.keys():
            ...
            camera_poses[cam_name] = {
                'pos': self.mj_model.cam_pos[cam_id].copy(),
                'quat': self.mj_model.cam_quat[cam_id].copy(),
            }
        return camera_poses
    ...
    def randomize_visuals_keep_camera_fixed(self) -> Dict[str, Any]:
    ...
    def evaluate_one_episode(self, episode_idx: int, seed: int) -> Dict[str, Any]:
        ...
        visual_info = self.randomize_visuals_keep_camera_fixed()
...
def build_argparser():
    parser = argparse.ArgumentParser(description='학습에 없던 새로운 object/bowl 위치 + texture/light를 생성해 LeRobot Diffusion Policy를 평가 (카메라 고정)')
